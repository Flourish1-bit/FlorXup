...
app.get('/admin/overview', (request, response) => {
  ...
  if (!user) return response.status(403).json({ error: 'Administrator access required.' });
...
app.delete('/admin/users/:id', async (request, response) => {
  const user = adminUser(request);
  if (!user) return response.status(403).json({ error: 'Administrator access required.' });
  if (request.params.id === user.id) return response.status(400).json({ error: 'The administrator cannot delete their own account.' });
